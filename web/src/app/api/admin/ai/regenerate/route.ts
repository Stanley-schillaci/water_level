...
// Le binaire est installé par uv au déploiement (lac-ai-refresh script).
// En prod : /var/lib/lac/.uv-cache/.../bin/lac-ai-refresh OU /opt/lac/worker/.venv/bin/lac-ai-refresh
// On laisse uv le résoudre via `uv run --no-sync lac-ai-refresh --force`.
...
export async function POST() {
  ...
  try {
    const child = spawn("uv", ["run", "--no-sync", "lac-ai-refresh", "--force"], {
      cwd: WORKER_DIR,
      detached: true,
      stdio: "ignore",
      env: process.env,
